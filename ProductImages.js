document.addEventListener("DOMContentLoaded", function () {
    try {
        // Function to wait for the main image element to exist with retries
        function waitForMainImageElement(callback, retries = 10) {
            const mainImageElement = document.getElementById("ctl00_PageBody_productDetail_ProductImage");
            if (mainImageElement) {
                callback(mainImageElement);
            } else if (retries > 0) {
                setTimeout(() => waitForMainImageElement(callback, retries - 1), 500);
            } else {
                console.warn("Main product image element not found after retries.");
            }
        }
        ...
        waitForMainImageElement(mainImageElement => {
