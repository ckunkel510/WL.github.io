$(document).ready(function () {
  ...
  $.get(sheetUrl, function (data) {
    ...
    if (!header || header.length === 0) {
      console.error("Failed to fetch or parse header row from Google Sheet.");
      return;
    }
    ...
    if (matchingRows.length === 0) {
      console.warn("No matching rows found for current productid:", currentPid);
      return;
    }
    ...
    matchingRows.forEach(row => {
      const optionType = row[optionTypeIndex]?.trim().toLowerCase() || "text";
