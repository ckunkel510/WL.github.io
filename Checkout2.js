  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    const steps = [
      ...
      },
      {
        ...
        findEls: () => {
          const a1 =
            document.getElementById("ctl00_PageBody_DeliveryAddress_AddressLine1") ||
            document.querySelector('[id*="DeliveryAddress_AddressLine1"]');
          const col = a1 ? a1.closest(".epi-form-col-single-checkout") : null;
          return col ? [col] : [];
        },
      },
      {
    ...
    function updatePickupModeUI() {
      ...
      setStepVisibility(3, delivered);   // Delivery Address step
      setDeliverySectionVisibility(delivered);

      // Keep hidden delivery fields satisfied in Pickup mode (WebTrack still validates them)
      try { wirePickupCopyFromBilling(); } catch (e) {}
      if (pickup) {
...
// Wire live syncing from Billing -> hidden Delivery fields in Pickup mode
function wirePickupCopyFromBilling() {
  ...
  invoiceFieldIds.forEach((id) => {
    ...
    if (!el) return;
    if (el.dataset && el.dataset.wlPickupSync === "1") return;

    el.dataset.wlPickupSync = "1";
    const handler = () => {
  ...
  // Also listen to the custom Pickup Person field (created later by the date/instructions module)
  const pickupPerson = document.getElementById("pickupPerson");
  if (pickupPerson && pickupPerson.dataset && pickupPerson.dataset.wlPickupSync !== "1") {
    pickupPerson.dataset.wlPickupSync = "1";
    const handler = () => {
      if (!getPickupSelected()) return;
      try { syncBillingToDelivery(); } catch (e) {}
    };
    pickupPerson.addEventListener("input", handler, true);
    pickupPerson.addEventListener("change", handler, true);
  }
}
...
// If a "Pickup Person" field exists, use it to populate delivery contact names
// (WebTrack can still validate delivery contact fields even in Pickup mode.)
const pickupPersonEl = document.getElementById("pickupPerson");
if (pickupPersonEl && getPickupSelected()) {
  const raw = (pickupPersonEl.value || "").trim();
  if (raw) {
    const parts = raw.split(/\s+/);
    const first = parts.shift() || raw;
    const last = parts.join(" ") || "Pickup";

    const dFirst = document.getElementById("ctl00_PageBody_DeliveryAddress_ContactFirstNameTextBox");
    const dLast  = document.getElementById("ctl00_PageBody_DeliveryAddress_ContactLastNameTextBox");
    if (dFirst && (!dFirst.value || !dFirst.value.trim())) dFirst.value = first;
    if (dLast  && (!dLast.value  || !dLast.value.trim()))  dLast.value  = last;
  }
}
}
...
// Keep step panes correct after UpdatePanel re-renders (Shipping change, branch change, address lookups, etc.)
function remountWizardSections() {
  try {
    steps.forEach((step, idx) => {
      ...
      const navDiv = pane.querySelector(".wl-step-nav");
      const els = (step && typeof step.findEls === "function") ? (step.findEls() || []) : [];
      els.forEach((el) => {
        if (!el) return;
        if (pane.contains(el)) return;
        if (navDiv) pane.insertBefore(el, navDiv);
        else pane.appendChild(el);
      });
    });

    // invoice fields can be re-rendered; re-wire once-per-element pickup sync
    try { wirePickupCopyFromBilling(); } catch (e) {}
  } catch (e) {
    // swallow remount errors so we don't break checkout
  }
}
window.WLCheckout.remountWizardSections = remountWizardSections;
    ...
    function hookAspNetAjax() {
      try {
        ...
        prm.add_endRequest(function () {
          ...
          reEnableWizardNav();
try { remountWizardSections(); } catch (e) {}
...
document.addEventListener("click", function (ev) {
  ...
  if (btn.dataset && btn.dataset.wlBack) {
    ...
    (function () {
      ...
      pickupDiv.querySelector("#pickupPerson").addEventListener("input", () => {
  updateSpecial();
  try { if (getPickupSelected()) syncBillingToDelivery(); } catch (e) {}
});
