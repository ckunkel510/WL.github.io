...
/* ============================================================
   ...
   - Prevent Billing "disappearing" by NOT moving server controls
   - Preserve wizard step during COF partial postbacks
   ============================================================ */
(function () {
  if (!/AccountPayment_r\.aspx/i.test(location.pathname)) return;
  if (window.__WL_WIZ_STABILIZER__) return;
  window.__WL_WIZ_STABILIZER__ = true;
  ...
  const IDs = {
    ...
    rbChk: "ctl00_PageBody_RadioButton_PayByCheck",
  };

  const $ = (id) => document.getElementById(id);

  function getStepRoot(stepNum) {
    // Works with your wizard variants
    return (
  ...
  function sanitizeEmail(val) {
    ...
    if (!raw) return "";
    const tokens = raw.split(/[\s,;<>()[\]]+/).filter(Boolean);
    return tokens.find((t) => /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(t)) || "";
  }

  // Build stable proxy Step 1 UI
  function ensureProxyUI() {
    const step0 = getStepRoot(0);
    if (!step0) return null;

    let proxy = step0.querySelector("#wlStep1Proxy");
    if (proxy) return proxy;

    proxy = document.createElement("div");
    proxy.id = "wlStep1Proxy";
    proxy.style.border = "1px solid #e5e7eb";
    proxy.style.borderRadius = "14px";
    proxy.style.padding = "12px";
    proxy.style.background = "#fff";
    proxy.style.display = "grid";
    proxy.style.gap = "10px";

    proxy.innerHTML = `
      <div style="font-weight:900;">Your information</div>

      <label style="display:grid;gap:6px;font-weight:800;">
        Email
        <input id="wlProxyEmail" type="email" style="padding:10px;border:1px solid #e5e7eb;border-radius:12px;" placeholder="name@email.com" />
      </label>

      <label style="display:grid;gap:6px;font-weight:800;">
        Address
        <select id="wlProxyAddrDropdown" style="padding:10px;border:1px solid #e5e7eb;border-radius:12px;"></select>
      </label>

      <label style="display:grid;gap:6px;font-weight:800;">
        Zip Code
        <input id="wlProxyPostal" inputmode="numeric" style="padding:10px;border:1px solid #e5e7eb;border-radius:12px;" placeholder="Zip" />
      </label>

      <label style="display:grid;gap:6px;font-weight:800;">
        Billing Address
        <textarea id="wlProxyBillingText" rows="3" style="padding:10px;border:1px solid #e5e7eb;border-radius:12px;" placeholder="Billing address"></textarea>
      </label>

      <label style="display:grid;gap:6px;font-weight:800;">
        Billing Zip
        <input id="wlProxyBillingZip" inputmode="numeric" style="padding:10px;border:1px solid #e5e7eb;border-radius:12px;" placeholder="Billing zip" />
      </label>

      <div style="font-size:12px;color:#475569;line-height:1.35;">
        These fields stay stable in the wizard. We sync them into the real WebForms fields automatically.
      </div>
    `;

    // Put proxy at top of Step 1
    step0.prepend(proxy);
    return proxy;
  }

  function hideRealGroups() {
    // We do NOT move these server controls. We only hide their wrappers to avoid duplicates.
    const els = [$(IDs.email), $(IDs.addrDropdown), $(IDs.postal), $(IDs.billText), $(IDs.billZip)].filter(Boolean);
    els.forEach((el) => {
      const wrap = el.closest(".epi-form-group-acctPayment") || el.closest("tr") || el.parentElement;
  ...
  function syncRealToProxy() {
    const proxy = ensureProxyUI();
    if (!proxy) return;

    const realEmail = $(IDs.email);
    const realAddr  = $(IDs.addrDropdown);
    const realPost  = $(IDs.postal);
    const realBill  = $(IDs.billText);
    const realBillZ = $(IDs.billZip);

    const pEmail = $("#wlProxyEmail");
    const pAddr  = $("#wlProxyAddrDropdown");
    const pPost  = $("#wlProxyPostal");
    const pBill  = $("#wlProxyBillingText");
    const pBillZ = $("#wlProxyBillingZip");

    // Email sanitize
    if (realEmail && pEmail) {
      const cleaned = sanitizeEmail(realEmail.value);
      if (cleaned) realEmail.value = cleaned;
      if (pEmail.value !== realEmail.value) pEmail.value = realEmail.value || "";
    }

    // Address dropdown options mirror
    if (realAddr && pAddr) {
      // rebuild options if different length/labels
      const ropts = Array.from(realAddr.options || []);
      const popts = Array.from(pAddr.options || []);
      const different =
        ropts.length !== popts.length ||
        ropts.some((o, i) => (popts[i]?.text || "") !== (o.text || ""));
      ...
      if (different) {
        ...
        ropts.forEach((o) => {
          ...
          pAddr.appendChild(opt);
        });
      }
      pAddr.value = realAddr.value;
    }

    if (realPost && pPost && pPost.value !== (realPost.value || "")) pPost.value = realPost.value || "";
    if (realBill && pBill && pBill.value !== (realBill.value || "")) pBill.value = realBill.value || "";
    if (realBillZ && pBillZ && pBillZ.value !== (realBillZ.value || "")) pBillZ.value = realBillZ.value || "";
  }
  ...
  function syncProxyToReal() {
    ...
    if (realEmail && pEmail) realEmail.value = sanitizeEmail(pEmail.value) || "";
    if (realAddr && pAddr) realAddr.value = pAddr.value;
    if (realPost && pPost) realPost.value = pPost.value || "";
    if (realBill && pBill) realBill.value = pBill.value || "";
    if (realBillZ && pBillZ) realBillZ.value = pBillZ.value || "";
  }

  function bindProxyEvents() {
    const proxy = ensureProxyUI();
    if (!proxy || proxy.__wlBound) return;

    ["wlProxyEmail","wlProxyAddrDropdown","wlProxyPostal","wlProxyBillingText","wlProxyBillingZip"]
      .map((id)=>document.getElementById(id))
      .filter(Boolean)
      .forEach((el)=>{
        el.addEventListener("input", ()=>{ syncProxyToReal(); }, { passive:true });
        el.addEventListener("change", ()=>{ syncProxyToReal(); }, { passive:true });
      });

    // When user clicks Next, make sure real fields have latest values
    const next = document.getElementById("wlWizNextBtn");
    if (next && !next.__wlProxyNextBound) {
      next.addEventListener("click", ()=>{ syncProxyToReal(); }, true);
      next.__wlProxyNextBound = true;
    }

    proxy.__wlBound = true;
  }

  // Preserve step on COF postback
  function rememberStep() {
    try {
      const current = Number(sessionStorage.getItem(STEP_KEY) || "0");
      sessionStorage.setItem(DESIRED_STEP_KEY, String(current));
    } catch {}
  }

  function bindCOFStepPreserve() {
    const rbCof = $(IDs.rbCof);
    const rbChk = $(IDs.rbChk);
    [rbCof, rbChk].filter(Boolean).forEach((rb)=>{
      if (rb.__wlStepBound) return;
      rb.addEventListener("mousedown", rememberStep, true);
      rb.addEventListener("click", rememberStep, true);
      rb.addEventListener("change", rememberStep, true);
      rb.__wlStepBound = true;
    });
  }

  function restoreStepAfterPostback() {
    let desired = 0;
    try { desired = Number(sessionStorage.getItem(DESIRED_STEP_KEY) || sessionStorage.getItem(STEP_KEY) || "0"); } catch {}
    if (!Number.isFinite(desired)) desired = 0;
    // If COF is checked, you almost always want to remain on Pay step
    const cof = $(IDs.rbCof);
    if (cof && cof.checked) desired = Math.max(desired, 3);
  ...
  try {
    if (window.Sys && Sys.WebForms && Sys.WebForms.PageRequestManager) {
      const prm = Sys.WebForms.PageRequestManager.getInstance();
      if (!prm.__wlStabilizerBound) {
        prm.add_endRequest(function () {
          // Re-hide real wrappers (they may get re-rendered)
          setTimeout(boot, 50);
          // Restore wizard step after re-mount
          setTimeout(restoreStepAfterPostback, 120);
          setTimeout(restoreStepAfterPostback, 280);
        });
        prm.__wlStabilizerBound = true;
      }
    }
  } catch {}
})();
