...
(function(){
  ...
  function pushProduct(products, id, qty) {
    ...
    else products.push({ productId: cleanId, quantity: quantity });
  }
  ...
  function buildSignature(products) {
    return products.map(function (item) { return item.productId + ":" + item.quantity; }).sort().join(",");
  }
  ...
  function setQuantity(data, doc, productId, quantity) {
    ...
    var field = null;
    for (var i = 0; i < selectors.length && !field; i++) field = doc.querySelector(selectors[i]);
    if (field && field.name) data.set(field.name, value);
    else data.set("ctl00$PageBody$productDetail$ctl00$qty_" + productId, value);
  }
  ...
  WL.MetaShop.run = async function(){
    try {
      ...
      const products = getRequestedProducts(params);
      if (!products.length) return;
      ...
      const beforeState = await getCartState("Before");
      ...
      const verified = await verifyCart(products, beforeState);
